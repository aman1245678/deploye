...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const revenueQuery = `
      SELECT
        (
          SELECT SUM(CAST(ro.quantity AS SIGNED) *
            CASE WHEN s.discountedPrice > 0 THEN s.discountedPrice ELSE s.price END)
          FROM retailer_orders ro
          JOIN stock s ON s.styleNo = ro.StyleNo
          WHERE ro.orderReceivedDate BETWEEN ? AND ? AND ro.status = 0
        ) AS retailer_revenue,
        (
          SELECT SUM(os.quantity *
            CASE WHEN s.discountedPrice > 0 THEN s.discountedPrice ELSE s.price END)
          FROM orders o
          JOIN orderStyles os ON os.orderId = o.id
          JOIN stock s ON s.styleNo = os.styleNo
          WHERE o.orderReceivedDate BETWEEN ? AND ? AND o.status = 0
        ) AS online_revenue
    `;
